...
struct TheaterList {
    ...
    func getLocation(num: Int) -> MKPointAnnotation {
        var annotation = MKPointAnnotation()
        annotation.coordinate = CLLocationCoordinate2D(
            latitude: mapAnnotations[num].latitude,
            longitude: mapAnnotations[num].longitude)
        annotation.title = mapAnnotations[num].location
        return annotation
    }
}
